...
# rootdir = [r"D:\PycharmProjects\djangoProject\app01\upload_file"]  # 以该路径为实验
...
def timetrans(str):
    global timetran
    ret = ""
    year = str[:str.index("年")]
    month = str[str.index("年") + 1:str.index("月")]
    ...
    if len(month) == 3:
        for i in range(len(month)):
            if month[i] == "十":
                ...
                ret = ret + timetran[month[i]]
    elif len(month) == 2:
        if month[0] == "十":
            ret = ret + "1" + timetran[month[1]]
        else:
            ret = ret + timetran[month[0]] + "0"
    else:
        if month[0] == '十':
            ret = ret + "10"
        else:
            ret = ret + "0" + timetran[month[0]]
    if len(day) == 3:
        for i in range(len(day)):
            if day[i] == "十":
                pass
            else:
                ret = ret + timetran[day[i]]
    elif len(day) == 2:
        if day[0] == "十":
            ret = ret + "1" + timetran[day[1]]
        else:
            ret = ret + timetran[day[0]] + "0"
    else:
        if day[0] == '十':
            ret = ret + "10"
        else:
            ret = ret + "0" + timetran[day[0]]
    return ret


# print(timetrans("二〇一八年四月一日"))
...
def find_defendent():
    ...
    for i in range(len(term_list)):
        if (str(term_list[i].word) == "被告人"):
            defendent = str(term_list[i + 1].word)
...
def save_as_json(filename):
    filename = filename[:-3].split("\\")[-1] + "json"
    verblist = []
    adjlist = []
    causesofActionlist = []
    victimslist = []
    ...
    # with open(r"C:\Users\Dudu\Desktop\分词json数据\hmm"+str(count)+".json",'w',encoding='utf-8') as json_file:
    with open(r"D:\PycharmProjects\djangoProject\app01\json_file" + "\\" + filename, 'w',
...
def reset_info():
    global defendent, defendentgender, defendentbirth, \
        ...
        causesofAction, adjs, verbs, model, count
    defendent = ""
    defendentgender = ""
    defendentbirth = ""
    defendentbirthplace = ""
